...
@Component({
  selector: 'app-showvhistorialc',
  standalone: true,
  imports: [CommonModule, ReactiveFormsModule, RouterOutlet, RouterModule],
  templateUrl: './showvhistorialc.component.html',
  styleUrl: './showvhistorialc.component.scss'
})
export class ShowvhistorialcComponent implements OnInit{
  lista_autos: any[] = [];
  ...
  getAutos(){
    this.http.consult_get('/admin/showreserva1').subscribe({
      next: (data: any) => {
        if(data.status){
