...
@Component({
  selector: 'app-crearrecepcion',
  standalone: true,
  imports: [CommonModule, ReactiveFormsModule, RouterOutlet, RouterModule],
  templateUrl: './crearrecepcion.component.html',
  styleUrl: './crearrecepcion.component.scss'
})
  ...
  registrar(){
    if (this.form_registro.valid) {
      if (this.form_registro.value.password === this.form_registro.value.confirm_password) {
        this.http.consult_post('/admin/registror', this.form_registro.value).subscribe({
          next: (data: any) => {
            if(data.status){
              alert(data.msg);
              //this.navigateLogin();
