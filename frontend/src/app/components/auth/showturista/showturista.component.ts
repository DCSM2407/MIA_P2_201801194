...
@Component({
  selector: 'app-showturista',
  standalone: true,
  imports: [CommonModule, ReactiveFormsModule, RouterOutlet, RouterModule],
  templateUrl: './showturista.component.html',
  styleUrl: './showturista.component.scss'
})
export class ShowturistaComponent implements OnInit{
  lista_turista: any[] = [];
  ...
  getUser(){
    this.http.consult_get('/admin/usuariost').subscribe({
      next: (data: any) => {
        if(data.status){
