...
@Component({
  selector: 'app-showreservavt',
  standalone: true,
  imports: [CommonModule, ReactiveFormsModule, RouterOutlet, RouterModule],
  templateUrl: './showreservavt.component.html',
  styleUrl: './showreservavt.component.scss'
})
export class ShowreservavtComponent implements OnInit{
  ...
  getReservas(usuario: any){
    ...
    this.http.consult_post('/admin/showreservavuelo', usuario).subscribe({
      next: (data: any) => {
        if(data.status){
